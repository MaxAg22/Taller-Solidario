...
export const orderFormSchema = z
  .object({
    ...
    newNotebooksSelected: z.array(z.string()).optional(),
  })
  .refine(
    (data) => {
      if (!data.newNotebooksSelected) return true;
      return data.newNotebooksSelected.length <= data.totalNotebooks;
    },
    {
      message:
        "La cantidad de notebooks seleccionados no puede ser mayor al total de notebooks",
      path: ["newNotebooksSelected"],
    }
  );
